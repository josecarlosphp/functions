...
function watermark($imagepath, $watermarkpath, $outputpath, $xAlign='middle', $yAlign='middle', $transparency=60, $proporcionW=0)
{
	...
	{
		switch(getExtension($watermarkpath))
		{
			case 'png':
				$function = 'imagecreatefrompng';
				break;
		...
		if(($imagew = $function($watermarkpath)))
		{
			...
			if($proporcionW && $proporcionW != (100 * $watermarkWidth) / $imageWidth) //round ?
			{
				...
				if(resizeImage($watermarkpath, array($watermarkWidth_new, $watermarkHeight_new), dirname($watermarkpath), basename($watermarkpath_new)))
				{
